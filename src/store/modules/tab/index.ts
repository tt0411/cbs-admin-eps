...
export const useTabStore = defineStore(SetupStoreId.Tab, () => {
  ...
  async function removeTab(tabId: string) {
    ...
    function update() {
      ...
      const filtered = dynamicRoutes.filter((r: any) => r.path !== tabId);
      if (filtered.length !== dynamicRoutes.length) {
        localStorage.setItem('dynamicRoutes', JSON.stringify(filtered));
       }
      }
